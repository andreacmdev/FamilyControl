import type { DespesaCategory, ReceitaCategory, EntryStatus, EntryType } from "@/types/database";

type CategoryConfig = { label: string; color: string; dot: string };

export const DESPESA_CATEGORY_CONFIG: Record<DespesaCategory, CategoryConfig> = {
...
export const RECEITA_CATEGORY_CONFIG: Record<ReceitaCategory, CategoryConfig> = {
  salario:     { label: "Salário",     color: "bg-emerald-100 text-emerald-700", dot: "bg-emerald-500" },
  freelance:   { label: "Freelance",   color: "bg-blue-100 text-blue-700",       dot: "bg-blue-400" },
  aluguel:     { label: "Aluguel",     color: "bg-amber-100 text-amber-700",     dot: "bg-amber-400" },
  bonus:       { label: "Bônus",       color: "bg-yellow-100 text-yellow-700",   dot: "bg-yellow-400" },
  rendimento:  { label: "Rendimento",  color: "bg-teal-100 text-teal-700",       dot: "bg-teal-400" },
  outros:      { label: "Outros",      color: "bg-zinc-100 text-zinc-600",       dot: "bg-zinc-400" },
};

/** Retorna a config de categoria para qualquer tipo de lançamento */
export function getCategoryConfig(category: string, entryType: EntryType): CategoryConfig {
  if (entryType === "receita") {
    return RECEITA_CATEGORY_CONFIG[category as ReceitaCategory] ?? RECEITA_CATEGORY_CONFIG.outros;
  }
  return DESPESA_CATEGORY_CONFIG[category as DespesaCategory] ?? DESPESA_CATEGORY_CONFIG.outros;
}
